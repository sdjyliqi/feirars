package control

//相关封装函数的UT
import (
	"github.com/sdjyliqi/feirars/testutil"
	"github.com/stretchr/testify/assert"
	"testing"
)

var TestUtil = CreatePingbackCenter(&testutil.TestCfg)

func Test_GetActiveDetailItems(t *testing.T) {
	item, count, err := TestUtil.GetActiveDetailItems(1, 10)
	assert.Nil(t, err)
	t.Log(item, count)
}

func Test_GetFeirarDetailItems(t *testing.T) {
	items, count, err := TestUtil.GetFeirarDetailItems(1, 10)
	assert.Nil(t, err)
	t.Log(items, count)
}

func Test_GetInstallDetailItems(t *testing.T) {
	items, count, err := TestUtil.GetInstallDetailItems(1, 10)
	assert.Nil(t, err)
	t.Log(items, count)
}

func Test_GetUninstallDetailItems(t *testing.T) {
	items, count, err := TestUtil.GetUninstallDetailItems(1, 10)
	assert.Nil(t, err)
	t.Log(items, count)
}

func Test_GetNewsDetailItems(t *testing.T) {
	items, count, err := TestUtil.GetNewsDetailItems(1, 10)
	assert.Nil(t, err)
	t.Log(items, count)
}

func Test_GetPreserveDetailItems(t *testing.T) {
	items, count, err := TestUtil.GetPreserveDetailItems(1, 10)
	assert.Nil(t, err)
	t.Log(items, count)
}
